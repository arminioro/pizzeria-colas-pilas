...
namespace laboratoriPizzeriaCampusExpress
{
	public partial class MainForm : Form
	{
		...
		private void BtnNuevoPedido_Click(object sender, EventArgs e)
		{
			...
			string clientePremium = txtCliente.Text;
			...
			if (clientePremium.StartsWith("PREMIUM:"))
			{
				colaPedidosPremium.Enqueue(clientePremium);
				
				pilaBitacora.Push(string.Format("PEDIDO PREMIUM: {0}", clientePremium));
				txtCliente.Clear();
				lblEstado.Text = string.Format("✅ Pedido PREMIUM registrado para {0}", clientePremium);
			}
			else
			{
				// Agregar a la cola
		...
		private void BtnEntregar_Click(object sender, EventArgs e)
		{
			...
			if (clientePremium.StartsWith("PREMIUM:"))
			{
				pilaBitacora.Push(string.Format("ENTREGADO: {0}", clientePremium));
		...
		private void BtnDeshacer_Click(object sender, EventArgs e)
		{
			...
			if (ultimaAccion.StartsWith("PEDIDO PREMIUM:"))
			{
				...
				string nombre = ultimaAccion.Replace("PEDIDO PREMIUM: ", "").Trim();
				// Reconstruir cola excluyendo ese pedido
				string[] temporal = colaPedidosPremium.ToArray();
		...
		private void ActualizarUI()
		{
			...
			foreach (string p in colaPedidosPremium)
				lstPedidosPremium.Items.Add(p);
			if (colaPedidosPremium.Count == 0)
				lstPedidosPremium.Items.Add("(Sin pedidos premium pendientes)");
			...
			// Actualizar contador
			lblContador.Text = string.Format("Pedidos: {0} | Pedidos Premium: {1} | Bitácora: {2}",
			                                 colaPedidos.Count, colaPedidosPremium.Count, pilaBitacora.Count);
		}
	}
}